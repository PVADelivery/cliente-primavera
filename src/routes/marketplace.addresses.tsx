    ...
    <div className="min-h-screen bg-background text-foreground pb-24">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6 space-y-6">
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogContent className="max-w-md w-[95vw] max-h-[85vh] overflow-y-auto rounded-3xl bg-background/95 backdrop-blur-2xl border border-white/10 p-0 scrollbar-thin">
            <DialogHeader className="p-6 pb-4 border-b border-white/5">
              <DialogTitle className="text-xl font-bold">{editing ? 'Editar Endereço' : 'Novo Endereço'}</DialogTitle>
              <DialogDescription className="sr-only">Preencha os campos abaixo.</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="p-4 pt-0 gap-3 border-t border-white/5 mt-2 bg-background/50">
              <Button variant="ghost" onClick={() => setShowForm(false)} className="rounded-xl h-12 w-full text-muted-foreground hover:text-foreground">Cancelar</Button>
              <Button onClick={handleSave} className="rounded-xl h-12 w-full font-bold bg-primary hover:bg-primary/90 text-primary-foreground">
