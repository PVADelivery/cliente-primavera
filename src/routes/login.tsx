...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Entrar — MT 24horas express" }] }),
  component: LoginPage,
});
    ...
    <div className="min-h-screen grid place-items-center px-6 bg-background relative">
      ...
      <div className="w-full max-w-sm space-y-6">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          <input type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="E-mail" className="w-full px-4 py-3 rounded-xl bg-card border border-border focus:outline-none focus:ring-2 focus:ring-ring" />
          <div className="relative">
            <input type={showPassword ? "text" : "password"} required autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Senha" className="w-full px-4 py-3 pr-12 rounded-xl bg-card border border-border focus:outline-none focus:ring-2 focus:ring-ring" />
